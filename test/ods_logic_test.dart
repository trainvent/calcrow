...
void main() {
  ...
  test(
    'ODS buildBytes keeps content.xml parseable for bundled fixture',
    () async {
      final sourceBytes = await rootBundle.load(
        'assets/test_objects/manipulate/Arbeitszeiten_2026.ods',
      );
  ...
  test(
    'ODS buildBytes keeps footer rows in place for bundled fixture',
    () async {
      final sourceBytes = await rootBundle.load(
        'assets/test_objects/manipulate/Arbeitszeiten_2026.ods',
      );
...
List<List<String>> _tableRows(XmlDocument document, String tableName) {
  ...
  final table = spreadsheet
      ...
      .firstWhere(
        (element) =>
            element.getAttribute('name', namespace: nsTable) == tableName,
      );

  return table.findElements('table-row', namespace: nsTable).map((row) {
    final values = <String>[];
